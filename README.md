...
Важный момент. Когда скачаете файл с проектом, вы не сможете сразу его запустить командой npm run dev. ДЛЯ НАЧАЛА ВАМ НАДО УСТАНОВИТЬ ЗАВИСИМОСТИ В ПАПКУ /client и в папку /server. чтобы установить зависимости надо ввести команду npm run install, в ОБЕ ПАПКИ	. После установки появится папка node_modules. Ее трогать не нужно. 
